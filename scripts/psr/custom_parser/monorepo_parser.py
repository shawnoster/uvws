...
class ConventionalCommitMonorepoParser(
    ...
    def __init__(self, options: ConventionalMonorepoParserOptions | None = None) -> None:
        ...
        self.mr_selector = regexp(r"[\t ]+\((?:pull request )?(?P<mr_number>[#!]\d+)\)[\t ]*$")
    ...
    def parse_message(self, message: str) -> ParsedMessageResult | None:
        ...
        level_bump = (
            ...
            else self.options.tag_to_level.get(parsed_type, self.options.default_bump_level)
        )
    ...
    def parse(self, commit: Commit) -> ParseResult:
        ...
        git_root = (
            Path(commit.repo.working_tree_dir or commit.repo.working_dir).absolute().resolve()
        )
